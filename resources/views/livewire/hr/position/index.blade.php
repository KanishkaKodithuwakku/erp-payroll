<div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
                            ...
                            <tr>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                                        <p class="text-theme-xs text-gray-500 dark:text-gray-400">Position Title</p>
                                    </div>
                                </th>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                                        <p class="text-theme-xs text-gray-500 dark:text-gray-400">Rate per Hour</p>
                                    </div>
                                </th>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                ...
                <div class="flex justify-between items-center border-t px-6 py-4 dark:border-gray-800">
                    <div class="text-sm text-gray-600 dark:text-gray-400">
                        Showing {{ $positions->firstItem() }} to {{ $positions->lastItem() }} of
                        {{ $positions->total() }} entries
                    </div>

                    <div class="pagination flex justify-between">
                       {{ $positions->links('vendor.pagination.custom-tailwind') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-data="{ show: @entangle('showForm') }" x-show="show" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-100 bg-opacity-40">
        <div class="relative w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
            <h3 class="mb-4 text-lg font-semibold" x-text="$wire.modalMode === 'add' ? 'Add Position' : 'Edit Position'"></h3>
            <form wire:submit.prevent="savePosition">
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-medium text-gray-700">
                        Position Title<span class="text-error-500">*</span>
                    </label>
                    ...
                    @error('position_name') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                </div>
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-medium text-gray-700">
                        Rate per Hour<span class="text-error-500">*</span>
                    </label>
                    <input type="number" step="0.01" wire:model.defer="rate_per_hour" 
                        class="w-full h-10 px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    @error('rate_per_hour') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                </div>
