<div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="mb-4 flex flex-col gap-4 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                        ...
                        <div class="relative">
                         ...
                         <button wire:click="createOvertime"
                            ...
                            <tr>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                                        <p class="text-theme-xs text-gray-500 dark:text-gray-400">Date</p>
                                    </div>
                                </th>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                                ...
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                                        <p class="text-theme-xs text-gray-500 dark:text-gray-400">No. of Hours</p>
                                    </div>
                                </th>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                                        <p class="text-theme-xs text-gray-500 dark:text-gray-400">Rate</p>
                                    </div>
                                </th>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                            ...
                            @foreach ($overtimes as $overtime)
                                <tr class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800">
                                    ...
                                    <td class="py-1 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <p class="text-theme-xs text-gray-700 dark:text-gray-400">
                                                {{ $overtime->hrEmployee->employee_id }}
                                            </p>
                                        </div>
                                    </td>
                                    <td class="py-1 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <p class="text-theme-xs text-gray-700 dark:text-gray-400">
                                                {{ $overtime->hrEmployee->firstname }} {{ $overtime->hrEmployee->lastname }}
                                            </p>
                                        </div>
                                    </td>
                                    <td class="py-1 whitespace-nowrap">
                                    ...
                                    <td class="py-1 whitespace-nowrap" style="width: 5%">
                                        <div class="flex items-center justify-start">
                                            <button wire:click="editOvertime({{ $overtime->id }})"
                                            ...
                                            <button wire:click="deleteOvertime({{ $overtime->id }})"
                ...
                <div class="flex justify-between items-center border-t px-6 py-4 dark:border-gray-800">
                    ...
                    <div class="pagination flex justify-between">
                       {{ $overtimes->links('vendor.pagination.custom-tailwind') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-data="{ show: @entangle('showForm') }" x-show="show" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-100 bg-opacity-40">
        <div class="relative w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
            <h3 class="mb-4 text-lg font-semibold" x-text="$wire.editingOvertime ? 'Edit Overtime' : 'Add Overtime'"></h3>
            <form wire:submit.prevent="saveOvertime">
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-medium text-gray-700">
                        Employee<span class="text-error-500">*</span>
                    </label>
                    <select wire:model.defer="employee_id" class="w-full h-10 px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option value="">Select Employee</option>
                        @foreach ($employees as $employee)
                            <option value="{{ $employee->id }}">{{ $employee->firstname }} {{ $employee->lastname }}</option>
                        @endforeach
                    </select>
                    @error('employee_id') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                </div>
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-medium text-gray-700">
                        Date<span class="text-error-500">*</span>
                    </label>
                    <input type="date" wire:model.defer="date_overtime" 
                    ...
                    @error('date_overtime') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                </div>
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">
                            Hours<span class="text-error-500">*</span>
                        </label>
                        <input type="number" wire:model.defer="hours" 
                            class="w-full h-10 px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                        @error('hours') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">
                            Minutes<span class="text-error-500">*</span>
                        </label>
                        <input type="number" wire:model.defer="minutes" 
                            class="w-full h-10 px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                        @error('minutes') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                    </div>
                </div>
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-medium text-gray-700">
                        Rate per Hour<span class="text-error-500">*</span>
                    </label>
                    <input type="number" wire:model.defer="rate" 
                        class="w-full h-10 px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    @error('rate') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                </div>
