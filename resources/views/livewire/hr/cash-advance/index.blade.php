<div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <table class="min-w-full">
                        <thead class="border-y border-gray-100 py-2 dark:border-gray-800">
                            <tr>
                                ...
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                                        <p class="text-theme-xs text-gray-500 dark:text-gray-400">Amount</p>
                                    </div>
                                </th>
                                <th class="py-2 font-normal whitespace-nowrap">
                                    <div class="flex items-center">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @forelse ($cashAdvances as $cashAdvance)
                                <tr class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800">
                                    <td class="py-1 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <p class="text-theme-xs text-gray-700 dark:text-gray-400">
                                                {{ $cashAdvance->date_advance->format('Y-m-d') }}
                                            </p>
                                        </div>
                                    </td>
                                    <td class="py-1 whitespace-nowrap">
                                    ...
                                    <td class="py-1 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <p class="text-theme-xs text-gray-700 dark:text-gray-400">
                                                {{ $cashAdvance->hrEmployee->full_name }}
                                            </p>
                                        </div>
                                    </td>
                                    <td class="py-1 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <p class="text-theme-xs text-gray-700 dark:text-gray-400">
                                                {{ number_format($cashAdvance->amount, 2) }}
                                            </p>
                                        </div>
                                    </td>
                                    <td class="py-1 whitespace-nowrap" style="width: 5%">
                                        <div class="flex items-center justify-start">
                                            <button wire:click="showEditModal({{ $cashAdvance->id }})"
                                            ...
                                            <button wire:click="deleteCashAdvance({{ $cashAdvance->id }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="py-4 text-center">No cash advances found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="flex justify-between items-center border-t px-6 py-4 dark:border-gray-800">
                    <div class="text-sm text-gray-600 dark:text-gray-400">
                        Showing {{ $cashAdvances->firstItem() }} to {{ $cashAdvances->lastItem() }} of
                        {{ $cashAdvances->total() }} entries
                    </div>

                    <div class="pagination flex justify-between">
                       {{ $cashAdvances->links('vendor.pagination.custom-tailwind') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-data="{ show: @entangle('showForm') }" x-show="show" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-100 bg-opacity-40">
        <div class="relative w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
            <h3 class="mb-4 text-lg font-semibold" x-text="$wire.modalMode === 'add' ? 'Add Cash Advance' : 'Edit Cash Advance'"></h3>
            <form wire:submit.prevent="saveCashAdvance">
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" wire:model.defer="date_advance" 
                        class="w-full h-10 px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    @error('date_advance') <p class="mt-1 text-xs text-error-500">{{ $message }}</p> @enderror
                </div>
